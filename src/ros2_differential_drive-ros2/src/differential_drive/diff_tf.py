...
class DiffTf(Node):
    ...
    def __init__(self):
        ...
        self.ticks_meter = float(
            self.declare_parameter("ticks_meter", 1018).value
        )  # The number of wheel encoder ticks per meter of travel / 50 --> 1018
        self.base_width = float(
            self.declare_parameter("base_width", 0.46).value
        )  # The wheel base width in meters / 0.245 --> 0.46
        ...
        self.odom_frame_id = self.declare_parameter(
            "odom_frame_id", "odom"
        ).value  # the name of the odometry reference frame /base_link --> base_footprint

        self.encoder_min = self.declare_parameter("encoder_min", -65536).value  # +- 32768 --> +- 65536
        ...
        # subscriptions
        self.create_subscription(ChannelValues, "hall_count", self.lwheel_callback, 10)
        self.create_subscription(ChannelValues, "hall_count", self.rwheel_callback, 10)
